...
const histogram = computed(() => {
  const values = (props.allScenarios || []).map(s => Number(s.totalChargeCapacityKw || 0))
  ...
  const current = Number(props.currentScenario?.totalChargeCapacityKw || 0)
  const currentBin = Math.min(BIN_COUNT - 1, Math.floor((current - min) / width))
  const labels = Array.from({ length: BIN_COUNT }, (_, i) => {
    const lo = (min + i * width) * 1000
    const hi = (min + (i + 1) * width) * 1000
    return `${formatSIWatts(lo)}-${formatSIWatts(hi)}`
  })
